# 2025-11-30 03:38:29 by RouterOS 7.20.2
...
add arp=proxy-arp dhcp-snooping=yes ingress-filtering=no name=bridge1 \
...
add dns=1.1.1.1,8.8.8.8,9.9.9.9 interface=MIHOMO3 mounts=proton_wg_conf name=\
    mihomo-mikrotik:latest remote-image=wiktorbgu/mihomo-mikrotik:latest \
    root-dir=usb1/docker/mihomo3 workdir=/
/container config
...
/ip firewall filter
add action=log chain=output dst-address=188.162.14.147 log-prefix=WG-REPLY: \
    protocol=udp
add action=log chain=ISP-Input dst-port=51820 log-prefix=WG-PEER1: protocol=\
    udp src-address=188.162.14.147
add action=passthrough chain=forward comment="LOOP: mihomo" dst-address=\
    192.168.254.3 src-address=192.168.254.3
add action=passthrough chain=forward comment="LOOP: mihomo2" dst-address=\
    192.168.254.4 src-address=192.168.254.4
...
add action=accept chain=GUEST-Input connection-state=established
add action=accept chain=GUEST-Input connection-state=related
add action=accept chain=GUEST-Input connection-state=untracked
add action=drop chain=GUEST-Input connection-state=invalid
add action=accept chain=GUEST-Input comment="Allow DNS" dst-port=53 protocol=\
    udp
add action=accept chain=GUEST-Input comment="Allow DHCP" dst-port=67 \
    protocol=udp
add action=drop chain=GUEST-Input comment="Drop all other to router"
add action=accept chain=VPN-Input connection-state=established
add action=accept chain=VPN-Input connection-state=related
add action=accept chain=VPN-Input connection-state=untracked
add action=drop chain=VPN-Input connection-state=invalid
add action=drop chain=VPN-Input
...
add action=accept chain=GUEST-Forward connection-state=established
add action=accept chain=GUEST-Forward connection-state=related
add action=accept chain=GUEST-Forward connection-state=untracked
add action=drop chain=GUEST-Forward connection-state=invalid
add action=accept chain=GUEST-Forward comment="Allow BlackList routes" \
    connection-mark=BlackList_RU
add action=accept chain=GUEST-Forward connection-mark=BlackList_EU
add action=drop chain=GUEST-Forward comment="Block RFC1918 + own subnet" \
    dst-address=10.0.0.0/8
add action=drop chain=GUEST-Forward dst-address=172.16.0.0/12
add action=drop chain=GUEST-Forward dst-address=192.168.0.0/16
add action=accept chain=GUEST-Forward comment="Allow to WAN" \
    out-interface-list=WAN
add action=drop chain=GUEST-Forward comment="Drop all other"
...
add action=accept chain=VPN-Forward connection-state=established
add action=accept chain=VPN-Forward connection-state=related
add action=accept chain=VPN-Forward connection-state=untracked
add action=drop chain=VPN-Forward connection-state=invalid
add action=accept chain=VPN-Forward comment="Allow new from VPN-OUT to WAN" \
    connection-state=new out-interface-list=WAN
add action=drop chain=VPN-Forward comment="Drop new from VPN-OUT" disabled=\
    yes
...
add action=dst-nat chain=dstnat comment="Video server ALL ports \?\?\?" \
    dst-address=10.101.101.200 log-prefix=DST-NAT: to-addresses=192.168.0.243
...
/ip route
add distance=1 dst-address=0.0.0.0/0 gateway=192.168.254.4 routing-table=\
    BlackList_EU
