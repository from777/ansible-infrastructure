...
### 28. Всегда указывать полный путь к плейбуку

**Правило:** При упоминании плейбука ВСЕГДА указывать полный путь от корня репозитория.

**Неправильно:**
```
Запусти Task: Install Zabbix Agent
```

**Правильно:**
```
Запусти Task: windows/install-zabbix-agent.yml
```

**Формат:** `папка/подпапка/имя-плейбука.yml`
...
### 28. Описание в начале каждого playbook — формат @тегов

**Правило:** Каждый playbook и task-файл ОБЯЗАТЕЛЬНО должен начинаться с метаданных в формате `# @тег значение`. Этот формат машинно-распознаваем (regex: `^# @(\w+)\s+(.+)$`).
...
|-----|-------------|----------|
| `@file` | да | имя файла |
| `@desc` | да | что делает плейбук (1-2 строки) |
| `@hosts` | да | группа из inventory |
| `@deps` | нет | зависимости — какие плейбуки запустить ДО этого |
| `@vars` | нет | какие переменные нужны (vault, extra vars) |
| `@notes` | нет | важные примечания |

**Полный пример:**
```yaml
# @file monitoring-kaspersky.yml
# @desc Создаёт template для мониторинга Kaspersky и привязывает к Windows хостам
# @hosts zabbix
# @deps setup-telegram.yml, setup-alert-action.yml
# @vars zabbix_user, zabbix_password (из group_vars/all.yml)
---
```
...
```yaml
# @file backup.yml
# @desc Создаёт бэкап конфигурации роутера
# @hosts mikrotik
---
```

**Для task-файлов (include_tasks) — без @hosts:**
```yaml
# @file cleanup.yml
# @desc Остановка и удаление старого контейнера
# @vars container_name, docker_bridge, smb_mount_point
---
```

**Правила:**
- Теги идут ПЕРВЫМИ строками файла
- Каждый тег на отдельной строке
- После тегов — `---` (начало YAML)
- Свободные комментарии (не @теги) допускаются ПОСЛЕ блока тегов
...
### 30. Именование папок — группы и служебные

**Правило:** Имя папки однозначно определяет её тип по первому символу.

| Начало имени | Тип папки | Содержимое |
|--------------|-----------|------------|
| без символа | группа в inventory | плейбуки, выполняются на хостах группы |
| `_` | служебная с плейбуками | плейбуки (include/import), не влияет на Limit |
| `~` | служебная только с файлами | скрипты, шаблоны, vars — НЕТ плейбуков |

**Примеры:**
```
mikrotik/                    # группа mikrotik
├── backup.yml
├── _containers/             # служебная, есть плейбуки
│   ├── _mihomo/             # служебная, есть плейбуки
│   │   ├── mihomo_awg.yml
│   │   └── ~templates/      # служебная, ТОЛЬКО файлы
│   │       └── config.yaml.j2
│   └── ~scripts/            # служебная, ТОЛЬКО файлы
│       └── check.sh
└── _firewall/               # служебная, есть плейбуки

linux/                       # группа linux
├── _install/                # служебная, плейбуки установки
│   ├── nginx.yml
│   └── ~templates/          # только шаблоны
└── ~scripts/                # только скрипты

zabbix/                      # группа zabbix
├── mikrotik/                # группа! zabbix + mikrotik в Limit
├── linux/                   # группа! zabbix + linux в Limit
└── _tasks/                  # служебная, include-файлы
```

**Как читать путь:**
- `mikrotik/_containers/_mihomo/mihomo_awg.yml` → только группа `mikrotik` (остальные с `_`)
...
3. После переименования — обновить все пути в плейбуках (`include_tasks`, `import_playbook`, `vars_files`)
